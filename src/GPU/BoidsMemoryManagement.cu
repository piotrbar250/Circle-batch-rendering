...
namespace cuda_functions
{
    ...
    void registerVBO(void **cuda_vbo_resource, unsigned int instanceVBO)
    {
        ...
        cudaError_t err = cudaGraphicsGLRegisterBuffer((cudaGraphicsResource **)cuda_vbo_resource, instanceVBO, cudaGraphicsMapFlagsWriteDiscard);
        if (err != cudaSuccess)
        {
    ...
    {
        cudaGraphicsResource *casted_cuda_vbo_resource = (cudaGraphicsResource *)cuda_vbo_resource;

        cudaGraphicsMapResources(1, &casted_cuda_vbo_resource, 0);
        printf("halosia\n");

        size_t num_bytes;
        glm::vec2 *devPtr;
        // cudaGraphicsResourceGetMappedPointer((void **)&devPtr, &num_bytes, (cudaGraphicsResource_t)(*casted_cuda_vbo_resource));
        printf("halosia2\n");
        return devPtr;
        exit(1);
        // cudaGraphicsResource **a = (cudaGraphicsResource **)cuda_vbo_resource;
